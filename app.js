...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addTelecomLayers() {
        ...
        if (!map.getSource('cell-towers')) {
            ...
            // Animated Outer Signal Wave (Geographically Scaled in Meters)
            map.addLayer({
                ...
                paint: {
                    'circle-radius': [
                        ...
                        ['zoom'],
                        0, 0,
                        10, ['*', ['get', 'coverage_radius_m'], 0.024],
                        12, ['*', ['get', 'coverage_radius_m'], 0.096],
                        14, ['*', ['get', 'coverage_radius_m'], 0.385],
                        16, ['*', ['get', 'coverage_radius_m'], 1.54],
                        18, ['*', ['get', 'coverage_radius_m'], 6.16],
                        20, ['*', ['get', 'coverage_radius_m'], 24.6]
                    ],
                    'circle-color': '#00f2fe',
                    'circle-opacity': 0.18,
        ...
        // 4. AI Proposed 5G Small Cells Layer (🟢 Green Pins)
    ...
    // Update Ookla Speed Tiles dynamically around towers
    function updateOoklaSpeedTiles(towers, globalMult) {
        ...
        speedTilesGeoJSON.features.forEach(tile => {
            ...
            let maxBoostedSpeed = tile.properties.original_speed_mbps || tile.properties.download_mbps;
            ...
            towers.forEach(t => {
                ...
                if (dist <= tRadius) {
                    const boost = Math.round((1 - (dist / tRadius)) * (simBw / 100) * 220 + 40);
            ...
            if (maxBoostedSpeed >= 120) tile.properties.color = "#00ff88"; // Fast 5G
            else if (maxBoostedSpeed >= 60) tile.properties.color = "#00b4d8"; // Medium 5G
            else if (maxBoostedSpeed >= 20) tile.properties.color = "#ffd166"; // 4G Moderate
            else tile.properties.color = "#ff3366"; // Slow / Dead Zone
        });
        ...
        // Dynamic H3 Hexagons Speed Upgrade
        if (h3HexagonsGeoJSON.features) {
            h3HexagonsGeoJSON.features.forEach(hex => {
                ...
                let maxBoostedSpeed = hex.properties.original_speed_mbps || hex.properties.download_mbps;
                ...
                towers.forEach(t => {
                    ...
                    if (dist <= tRadius) {
                        const boost = Math.round((1 - (dist / tRadius)) * (simBw / 100) * 240 + 45);
                        if (boost > maxBoostedSpeed) {
                            maxBoostedSpeed = boost;
                        }
                    }
                ...
                if (maxBoostedSpeed >= 120) hex.properties.color = "#00ff88";
                else if (maxBoostedSpeed >= 60) hex.properties.color = "#00b4d8";
                else if (maxBoostedSpeed >= 20) hex.properties.color = "#ffd166";
                else hex.properties.color = "#ff3366";
            });
    ...
    function setupInteractivity() {
        ...
        map.on('click', 'layer-towers', (e) => {
            ...
            new maplibregl.Popup()
                .setLngLat(e.lngLat)
                .setHTML(`
                    <div class="popup-title">${props.is_user_deployed ? '🟡 User-Deployed 5G Cell' : '🔴 Legacy 4G/3G Tower'}</div>
                    <div class="popup-row"><span class="label">Name:</span> <span class="val">${props.name}</span></div>
                    <div class="popup-row"><span class="label">Technology:</span> <span class="val">${props.technology}</span></div>
                    <div class="popup-row"><span class="label">Tx Power:</span> <span class="val">${props.tx_power_w || 40} W</span></div>
                    <div class="popup-row"><span class="label">Coverage Radius:</span> <span class="val">${Math.round((props.coverage_radius_m || 480) * (props.radius_multiplier || 1.0))} m</span></div>
                    <div class="popup-row"><span class="label">Status:</span> <span class="val" style="color:${props.is_user_deployed ? '#ffd166' : '#00ff88'}">${props.status}</span></div>
                    ${deleteBtnHtml}
                `)
                .addTo(map);
        });
